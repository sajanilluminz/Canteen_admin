...
import {PatchApiCall, postApiCall} from '~adapters/ApiManager';
...
const UpdateProduct = ({route}: any) => {
  ...
  var updatedProduct: IProductsProps;
  const saveChanges = (item: {
    ...
    if (item.quantity?.trim().length === 0) {
      ...
      if (
        ...
        return PatchApiCall({
          url: CanteenEndpoint.updateProduct,
          data: params,
          headers,
        })
          .then(res => {
            ...
            updatedProduct = {
              name: response.name,
              category: response.categoryId.name,
              price: response.price,
              id: response._id,
              quantity: response.quantity,
              isDeleted: response.isDeleted,
              image: `${baseUrl}${response.image.image_url}`,
